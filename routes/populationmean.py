...
def calculate_ci():
    ...
    try:
        ...
        ax.plot(x, y1, 'b-')
        ax.plot(x, y, 'b-')
        ax.scatter(lower_bound, 0, color='red', alpha=0.99, label='Lower Bound')
        ax.scatter(upper_bound, 0, color='red', alpha=0.99, label='Upper Bound')
        ax.scatter(mu0, 0, color='green', alpha=0.99, label='Sample Mean (x̄)')
    ...
    return jsonify({'ci_result': ci_result, 'plot': encoded})
...
def sigmaknown():
    ...
    try:   
        ...
        if test_type == "two-tailed":
            ...
            df = pd.DataFrame({"Z Test Statistic": [z_test], "Critical Z Value": [z], "P-Value": [p_value], "alpha": [alpha],
